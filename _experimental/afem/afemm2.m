%
% AFEMM2 : Adaptive Finite Element Method in Matlab
%
% Calling Convention:
%    function afem = afemm2(P0, T0, fem, fixbdry, INFO)
%
%

%
% TODO
%  o Equidistribution refinement + coarsening
%  o properties of elements (and edges???) (???)
%  o An algorithm that checks the initial mesh for admissibility
%

%
% List of things that should be modified for a "release" version
% versus a high-speed version.
%  o myfind should be replaced by straight matlab.
%  o 
%
...
% get empty afem object
afem = empty_afemm2(fem, fixbdry);
...
afem = edges_init(afem);
% create edge markers
afem.mark = zeros(1, afem.nE, 'int8');
...
% for later, save also nP0 and nT0
afem.nP0 = afem.nP;
afem.nT0 = afem.nT;
...
%
% perform nRef uniform refinements
%
function afem = refineUniform(afem, nRef)
for n = 1:nRef
  afem.mark = afem.mark * 0 + 1;
  afem = afem.refine(afem);
end
end
...
%
% random refine: performs n random refinement loops.
% n times : each edge is marked with probability prob
%
function afem = randomRefine(afem, nRef, prob)

for n = 1:nRef
  p = rand(size(afem.mark));
  afem.mark(p <= prob) = 1;
  afem = afem.refine(afem);
end
...
%
% Transfer marked elements to marked edges
%
...
nT = afem.nT;
afem.aTE = [ J(1:nT), J((nT+1):(2*nT)), J((2*nT+1):(3*nT)) ]';
...
afem.aET = zeros(2, size(E, 2));
for k = 1:nT
  t = afem.T(:, k); t = [t; t(1)];
  for j = 1:3
    % case 1: left
    if afem.E(1, afem.aTE(j, k)) == t(j+1)
      afem.aET(1, afem.aTE(j, k)) = k;
    % case two: right
    else
      afem.aET(2, afem.aTE(j, k)) = k;
    end
  end
end
...
iE = find(afem.aET(1, :) == 0);
afem.E(:, iE) = afem.E([2,1], iE);
afem.aET(:, iE) = afem.aET([2,1], iE);
